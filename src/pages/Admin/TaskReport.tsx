    ...
    const fetchData = async () => {
      try {
        ...
        const pointsRes = await axios.get<{ points: StudentPoint[] }>(
          `http://localhost:8000/api/admin/manage_task_points/${event_id}/${task_id}/`,
          {
            headers: { Authorization: `Bearer ${token}` },
            withCredentials: true,
          },
        ...
        if (!pointsRes.data.points || pointsRes.data.points.length === 0) {
          ...
          try {
            const studentsRes = await axios.get<{ students: Student[] }>(
              `http://localhost:8000/api/admin/getstudent_task_report/${event_id}/${admin_id}/`,
              {
                headers: { Authorization: `Bearer ${token}` },
                withCredentials: true,
              },
  ...
  const handleSubmitPoints = async (contextId: string) => {
    try {
      ...
      const submitResponse = await axios.post(
        `http://localhost:8000/api/admin/manage_task_points/${task.event_id}/${task.task_id}/`,
        payload,
        {
          headers: {
